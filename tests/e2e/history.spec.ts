...
test.describe("History / transactions view", () => {
  test("current month expenses appear in history", async ({ page, request }) => {
    const budget = await seedBudget(request);
    await addExpense(request, { amount: 120, description: "History test expense", categoryId: budget.categoryId });

    await page.goto("/history");
    await expect(page.locator('text=History test expense')).toBeVisible();
  });

  test("expenses show under the correct month", async ({ page, request }) => {
    const budget = await seedBudget(request);
    await addExpense(request, { amount: 99, description: "April only expense", categoryId: budget.categoryId });
    ...
    await page.locator('button[aria-label="Previous month"], button:has-text("‹"), button:has-text("<")').first().click();
    await expect(page.locator('text=April only expense')).not.toBeVisible();
  });

  test("recurring expense is tagged in history view", async ({ page, request }) => {
    const budget = await seedBudget(request);
  ...
  test("previous months do not leak into current month view", async ({ page, request }) => {
