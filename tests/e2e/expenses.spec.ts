...
test.describe("One-time expenses", () => {
  ...
  test("add expense via floating button", async ({ page, request }) => {
    ...
    await page.locator('[data-testid="expense-category-select"]').selectOption({ label: /Test Category/ });
  ...
  test("edit an existing expense", async ({ page, request }) => {
    const budget = await seedBudget(request);
